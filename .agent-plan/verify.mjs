...
} else if (await stat(resolve(repositoryRoot, ".agent-plan", "capture-proof-dashboard.mjs")).then(() => true, () => false)) {
...
if (evidenceDirectory) {
  // Keep dashboard inspection captures from overwriting the criterion proof manifest.
  const dashboardDirectory = resolve(repositoryRoot, evidenceDirectory, "dashboard");
  ...
  const result = await runCheck({ ...capture, env: { ...process.env, AGENT_PLAN_EVIDENCE_DIR: dashboardDirectory } });
  if (result.error || result.code !== 0) {
    failed = true;
    process.stderr.write(`Failed ${capture.name}${result.error ? `: ${result.error.message}` : result.signal ? `: terminated by ${result.signal}` : ""}\n`);
  } else {
    const outputDirectory = dashboardDirectory;
