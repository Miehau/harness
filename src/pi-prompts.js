...
export const discoveryInstruction = `Start at .agent-plan/feature-map.md, then read only the relevant feature leaf and owning source. Maintain this progressive map when behavior or navigation changes: a small feature index, directory ownership, and linked leaves explaining purpose, UI entry points, CLI journeys, observable states, and focused tests. Reuse existing feature documentation through links instead of duplicating it. Before implementation handoff, update the affected feature leaves and any changed index links in the same change; report the map paths updated, or explain why the existing map remains accurate. If required documentation is outside the declared write scope, report needs_input instead of silently skipping it.
...
- For a small low-risk change, use one vertical product slice including its tests, documentation and proof tooling; retain only genuinely required architecture prerequisites. Do not create separate slices solely for adding a dependency, wiring a test, or capturing proof. Never narrow the requested outcome to fit this preference.
- Include reviewPolicy: {"mode":"small","reason":"why this is bounded and low risk","risks":[]} only for a small single-repository change without security boundaries, credentials, destructive operations, migrations, concurrency or external integration changes. Otherwise use mode=standard and list the risks. The approved policy selects one comprehensive independent reviewer for small diffs; larger diffs retain specialist reviews.
- Criterion bindings may declare scope="step" for temporary isolation constraints that hold only when that step is accepted (for example, no product package yet). Use scope="final" for every required finished-product outcome; omission defaults to final. Never use step scope to exempt a product requirement from final verification.
...
- The verification bootstrap creates project.json, feature-map.md, ui.mjs and ui.test.mjs inside .agent-plan. Declare a named install argv command only when a dependency manifest already exists, using its package manager and lockfile; managed preparation runs it privately when manifests change. In an empty repository, bootstrap checks must run with built-ins without installing packages or requiring product files from later steps. The step that first adds dependencies owns enabling installation and must include project.json in its write scope. Store executable commands as argv arrays in project.json; never parse prose for commands. ${discoveryInstruction} Each feature slice owns its affected map leaves, navigation commands and tests within its declared write scope. Keep writes to shared CLI/index files serial. Bootstrap must not modify product code or agent guidance.
