...
2. Run `orchestrator brief <ticketId> <runId>` when reporting progress or reconnecting. It returns the same versioned fields as `show`, plus a readable `message` covering status, pending questions, UI impact and proposal revision. Present `metrics` separately; keep unavailable and partial cost labels. Never turn missing cost into zero or describe SDK-reported cost as an invoice.
