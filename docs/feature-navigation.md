...
| **F14** Dashboard / CLI / observability | Ticket queue → filter → ticket → stage / worker inspector. Activity is streamed; Prompt/Artifacts/Details/Cleanup expose persisted evidence. Timeline and review packet name each configured repo. | `GET /api/events` (SSE), `/api/state`, `T/run`, `T/inspection`, `T/stages/:stageId/prompts`, `S/session-trace`; `select <ticketId>`, `status [ticketId]`, `list timeline [ticketId]`, `wait [ticketId]`, `access show|set`. | [app.js](../public/app.js): `render`, `selectTicket`, `renderInspector`, EventSource handlers; [ui-model.js](../public/ui-model.js): `eventTimeline`; [inspection.js](../src/inspection.js); [activity.js](../src/activity.js): `createActivityCapture`; [inspection.js](../src/inspection.js): `publicRun`, `publicState`; [cli.js](../src/cli.js): `runCli`; [http.js](../src/http.js). | `cli ui-model markdown server scripts inspection multi-repo-flow` |
