...
|---|---|---|---|---|
| **F01** Workspace / credentials | Top bar **Repository** (`#workspace-settings`) → Choose / Open repository; `#access-policy-form` (`#access-any`, `#extra-roots-list`, `#save-access-policy`). Queue footer **Trackers** (`#tracker-settings`). | `/api/workspace`, `/api/workspace/pick`; `GET` + `POST /api/workspace/access-policy`; `GET` + `POST /api/tracker-settings`; `access show|set`. | [server.js](../src/server.js): `createDaemon`, `trackerHub`; [operator-services.js](../src/operator-services.js): workspace and settings operations; [access-policy.js](../src/access-policy.js): `normalizeProjectPolicy`, `freezeRunAccess`; [credentials.js](../src/credentials.js): `CredentialStore`, `effectiveTrackerCredentials`; [http.js](../src/http.js). | `access-policy credentials daemon-lock server dashboard cli` |
| **F02** Ticket intake / admission | **New** (`#free-text-open`) → description → Start workflow. Or Refresh → select tracker ticket → start. Agent setup → Project mode enables automatic intake. | `T/start` with `{ticket}`; `/api/tickets/start`; `GET /api/tickets`; `new text <prompt>`, `start <ticketId>`, `list backlog`, `orchestrator submit <json\|@file\|->` (draft only). | [ticket-runner.js](../src/ticket-runner.js): `beginTicket`; [server.js](../src/server.js): tracker polling and admission; [admission.js](../src/admission.js): `admissionCandidates`; [trackers.js](../src/trackers.js): `TrackerHub`; [ui-model.js](../public/ui-model.js): `freeTextTicket`. | `admission trackers linear jira e2e-automation cli orchestration` |
| **F03** Requirements / exploration | Select ticket → requirements questions in header → answer / approve; select Requirements or Explore stage → Activity, Prompt, Artifacts, Details. | `T/clarify` with `{answers}`; `answer <ticketId> <text>` or `answer <ticketId> --approve` when no unanswered required questions remain. | [planning.js](../src/planning.js): `prepareTicket`, `continueAfterRequirements`, `designTicket`; [ticket-runner.js](../src/ticket-runner.js): `acceptCheckpointAnswer`; [pi-harness.js](../src/pi-harness.js): `PiHarness`; [execution.js](../src/execution.js): `runStageDefs`. | `pi-flow e2e-daemon server` |
| **F04** Plan edit / approval | Plan approval checkpoint → **Edit graph JSON** → JSON → Save plan; **Run manually** or auto action. Gate fixture: `plan-approval`. | `T/plan` with `{plan}`; `T/approve` with `{auto}`; `approve [ticketId] [--auto]`. CLI has no edit-plan command. | [plan.js](../src/plan.js): `normalizeEditedPlan`, `planReviewViolations`, `blockingReasons`; [execution.js](../src/execution.js): `planApprovalPending`; [ticket-runner.js](../src/ticket-runner.js) owns approval transitions; [routes.js](../src/routes.js) parses requests. | `plan execution server` |
| **F05** Profiles / skills / workflows | Top-bar **Agent setup** (`#profile-settings`) → models / reasoning / instructions; workflow gates appear in ticket header. Plan approval discloses that named commands are not a filesystem sandbox. Some advanced operations are API/CLI only. | `GET /api/models`, `/api/skills`, `T/skills`; `/api/stage-profiles`; `T/stage-profiles/:profileId`; `T/workflow`, `T/workflow/continue`; `profile <stage> <model> <thinking> [ticketId]`. | [profiles.js](../src/profiles.js): `normalizeStageProfiles`; [workflow.js](../src/workflow.js): `applyPendingWorkflowGate`, `applyWorkflowContinuation`; [pi-tools.js](../src/pi-tools.js): `scopedWorkerTools`, `scopedReadTools`; [ticket-runner.js](../src/ticket-runner.js): workflow continuation; Pi `activateWorkflow`. | `profiles profile-override workflow pi-harness server` |
| **F06** Isolated execution / VCS | Approve plan → implementation graph → select worker → Activity / Details; inspector footer identifies worktree and jj revision. Extra Git roots appear as additional `run.repositories` worktrees. | `T/approve`, `T/resume`, `S/accept`; `approve`, `resume`, `accept`. Execution is orchestration, not a separate public launch-worker route. | [ticket-runner.js](../src/ticket-runner.js): `runTicket`, `advanceTicket`; [step-runner.js](../src/step-runner.js): `executeStep`, `acceptStep`; [execution.js](../src/execution.js): `nextRunnableBatch`; [worktrees.js](../src/worktrees.js): `ensureTicketWorktree`, `createParallelWorktrees`, `gitRepositoriesForStep`, `mapConfiguredPath`, `integrateBranch`; [git.js](../src/git.js), [jj.js](../src/jj.js). | `execution worktrees git jj pi-flow e2e-daemon multi-repo-flow` |
| **F07** Step review / criterion proof / diffs | Select review-ready step → Diff or Details → inspect criteria → Accept commit / Request changes. Diff includes semantic review-map action and queued section notes. Fixture: `review-ready`. | `S/accept`, `S/changes`, `S/review-map`; `GET T/proof/diff`, `T/proof/check-output`, `T/review-packet`; `accept <stepId> [ticketId] [--auto]`; see [correction payloads](#correction-payloads). | [proof-map.js](../src/proof-map.js): `initializeProofMap`, `proofEligibility`, `resolveEvidence`; [ui-model.js](../public/ui-model.js): `proofMapView`; [app.js](../public/app.js): `stepDiffPanel`, `criterionProofHtml`, `renderInspector`; [inspection.js](../src/inspection.js): canonical diff projections. | `proof-map criterion-proof-flow review-packet ui-model e2e-proof` |
| **F08** Verification / correction | Worker Activity / Details shows checks and correction history; a blocked verifier can be resumed, scope-expanded or waived through CLI. Combined checks run per changed Git worktree and fail the gate if any repository fails. | `S/changes`, `S/scope` with `{paths,reason}`, `S/waive` with `{reason}`; `scope-add <stepId> <ticketId> <path> <reason>`, `waive <stepId> <ticketId> <reason>`. | [project-config.js](../src/project-config.js): `runProjectCommand`, `runManagedCommand`; [pi-tools.js](../src/pi-tools.js): `projectCommandTool`, `scopedWorkerTools`; [pi-prompts.js](../src/pi-prompts.js): `ensureVerificationContractStep`; [git.js](../src/git.js): `combineRepositoryChecks`; [preview-orchestration.js](../src/preview-orchestration.js): `runChecksWithPreview`; [final-review.js](../src/final-review.js): `finalReviewLoop`; [repository-checks.js](../src/repository-checks.js); execution `shouldPauseCorrection`. | `project-config verification-contract pi-harness process-containment proof-restarts multi-repo-flow` |
| **F09** Preview / visual evidence | Ticket header → start / stop live preview; verification artifacts show screenshots or recordings. | `T/preview` with `{action:"start"}` or `"stop"`; `GET T/artifacts/:artifactId/media`; `preview start|stop [ticketId]`. | [previews.js](../src/previews.js): `PreviewManager`; [visual-evidence.js](../src/visual-evidence.js): `applyVerifyEvidenceGate`, `ticketBoundVisualEvidence`; [preview-orchestration.js](../src/preview-orchestration.js): `startOperatorPreview`, `runChecksWithPreview`; [artifacts.js](../src/artifacts.js). | `previews visual-evidence artifacts e2e-proof` |
| **F10** Final proof approval | Evidence-review checkpoint → inspect proof / tests / screenshots → **Approve & deliver** or Request changes with affected criteria. Fixture: `proof-review`. | `T/evidence/approve`, `T/evidence/changes`, `T/review-fix/restart`; `approve-proof [ticketId]`, `restart-fixer <ticketId> <reason>`; `T/context/approve` is a compatibility alias to the same gate. | [final-review.js](../src/final-review.js): `completeCleanReview`, `applyFinalReviewFix`, `finalReviewLoop`; [ticket-runner.js](../src/ticket-runner.js): `finishHandoff`; [proof-revision.js](../src/proof-revision.js); execution `restartReviewFixSession`, `recoverableCleanReview`; proof-map `invalidateProof`. | `e2e-proof proof-restarts criterion-proof-flow server` |
| **F11** Delivery / tracker writeback | After final proof approval → Handoff Activity / artifacts / review packet. Per-repo `run.deliveries` progress asynchronously; partial failure names `displayPath`. | `T/evidence/approve` starts downstream delivery; `approve-proof [ticketId]`; `resume` retries unfinished repos. `status <ticketId>` / `GET T/run` observes it. | [delivery-runner.js](../src/delivery-runner.js): `scheduleAllDeliveries`, `fixRemoteFeedback`; [ticket-runner.js](../src/ticket-runner.js): delivery dispatch; [delivery.js](../src/delivery.js): `changedGitDeliveryRepos`, `GitHubDelivery`, `GitLabDelivery`, `safeSyncLocal`; [merge-queue.js](../src/merge-queue.js): `enqueueSerial`; [linear.js](../src/linear.js), [jira.js](../src/jira.js). | `delivery merge-queue e2e-automation linear jira multi-repo-flow` |
| **F12** Recovery / process cleanup | Header Pause / Resume / Restart; Restart dialog selects saved point and requires confirmation. Stage or worker → Cleanup. Fixtures: `interrupted`, `needs-attention`. | `T/pause`, `T/resume`, `T/cancel`, `T/restart`; `pause`, `resume`, `cancel`; `restart <ticketId> [target] --confirm`. Targets: `fresh`, `stage:explore`, `stage:design`, `stage:verify`, or valid `step:<id>`. | [ticket-runner.js](../src/ticket-runner.js): `pauseTicket`, `resumeTicketPipeline`, `restartFrom`, `startFreshRun`; [run-runtime.js](../src/run-runtime.js): containment cleanup; [server.js](../src/server.js): `close`; execution `rewindRun`, `prepareRunResume`, `completeRunCleanup`; [process-containment.js](../src/process-containment.js), [process-tree.js](../src/process-tree.js); [store.js](../src/store.js). | `restart run-status process-cleanup-e2e process-containment process-tree store` |
| **F13** Artifacts / retention | Stage or worker → Artifacts → select artifact / open externally. Top bar storage icon (`#retention-open`) → select retained runs → Clean selected → confirm. Extra Git worktrees are inventoried with the primary. | `GET T/artifacts/:artifactId`, `T/artifacts/:artifactId/open`; `GET /api/retention`, `/api/retention/cleanup` with `{ticketIds,confirmed:true}`; `T/forget` with confirmation; `/api/queue/clear`; `queue clear`. | [artifacts.js](../src/artifacts.js): `persistArtifact`, `hydrateArtifact`, `artifactPathInDataDir`; [retention.js](../src/retention.js): `retentionInventory`, `cleanupRetainedRun` (each repo `sourceCwd`); execution `archiveRun`, `clearInactiveRuns`; [store.js](../src/store.js): `JsonStore`. | `artifacts retention store server` |
| **F14** Dashboard / CLI / observability | Ticket queue → filter → ticket → stage / worker inspector. Activity is streamed; Prompt/Artifacts/Details/Cleanup expose persisted evidence. Timeline and review packet name each configured repo. | `GET /api/events` (SSE), `/api/state`, `T/run`, `T/inspection`, `T/stages/:stageId/prompts`, `S/session-trace`; `select <ticketId>`, `status [ticketId]`, `list timeline [ticketId]`, `wait [ticketId]`, `access show|set`, `orchestrator show\|brief <ticketId> <runId>`, `orchestrator act <ticketId> <json\|@file\|->`. | [app.js](../public/app.js): `render`, `selectTicket`, `renderInspector`, EventSource handlers; [ui-model.js](../public/ui-model.js): `eventTimeline`; [inspection.js](../src/inspection.js); [activity.js](../src/activity.js): `createActivityCapture`; [inspection.js](../src/inspection.js): `publicRun`, `publicState`; [cli.js](../src/cli.js): `runCli`; [orchestration.js](../src/orchestration.js): `createOrchestratorService`; [http.js](../src/http.js). | `cli orchestration ui-model markdown server scripts inspection multi-repo-flow` |
